...
if __name__ == "__main__":
    ...
    train_dataloader = DataLoader(training_set, shuffle=False, collate_fn = data_collator, batch_size=args.per_device_train_batch_size)
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        docs_dict = retriever(batch["input_ids"].cpu().numpy(), question_hidden_states["pooler_output"].detach().cpu().numpy(), batch["allowed_doc_ids"].tolist(), return_tensors="pt", n_docs=args.n_docs, cited_abstract_only = args.cited_abstract_only)
        retrieved_doc_ids.extend(docs_dict["doc_ids"].tolist())
        progress_bar.update(1)

    with open(args.output_dir,"w", encoding='utf8') as f:
        for doc_ids, data in zip(retrieved_doc_ids, training_dataset):
            this_data = copy.deepcopy(data)
            this_data["cited_indices"] = doc_ids
