...
def parse_args():
    ...
    parser.add_argument(
        "--train_file", type=str, default="/home/data/XiangciLi/CORWA/annotated_train", help="A csv or a json file containing the training data."
    )
    
    parser.add_argument(
        "--distant_file", type=str, default=None, help="A csv or a json file containing the training data." # "/home/data/XiangciLi/CORWA/CORWA_distant"
    )
    
    parser.add_argument(
        "--validation_file", type=str, default="/home/data/XiangciLi/CORWA/annotated_test_Nov15", help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--exclude_intro", action="store_true"
    )
...
class SimpleDataLoader():
    def __init__(self, dataset, original_dataset, shuffle=False):
    ...
    def __iter__(self):
        ...
        for idx in indices:
            ...
            yield self.__wrap_batch(current_data), self.original_dataset[idx]
def main():
    ...
    validation_set = CitationTextGenerationDataset(
         args.validation_file, tokenizer,
         MAX_SENT_LEN=args.max_source_length,
         include_conclusion=False,
         include_intro=not args.exclude_intro,
         ...
         include_cited_paper = not args.exclude_cited_paper
    )
    
    training_set = CitationTextGenerationDataset(args.train_file, tokenizer,
                                         MAX_SENT_LEN=args.max_source_length,
                                         include_conclusion=False,
                                         include_intro=not args.exclude_intro,
                                         skip_no_citations=False,
    ...
    if args.distant_file is not None:
        distant_dataset = CitationTextGenerationDataset(args.distant_file,
                                                        tokenizer,
                                                        MAX_SENT_LEN=args.max_source_length,
                                                        include_conclusion=False,
                                                        include_intro=not args.exclude_intro,
    ...
    train_dataset_ids = train_dataset["id"]
    eval_dataset_ids = eval_dataset["id"]
    ...
    train_dataloader = SimpleDataLoader(train_dataset, training_set, shuffle=True)
    ...
    #maml = l2l.algorithms.MAML(model, lr=args.fast_learning_rate, first_order=True)
    ...
    for epoch in range(args.num_train_epochs):
        model.train()
        for step, (data, original_data) in enumerate(train_dataloader):
            outputs = model(**data)
            loss = outputs.loss
        ...
        predictions = []
        gold_labels = []
        for step, (data, original_data) in tqdm(enumerate(eval_dataloader)):
            citation_mark = validation_set[step]["citations"]
        ...
        if args.dominant_only:
            name = "standard_training_eval_predictions_dominant.jsonl"
        else:
            name = "standard_training_eval_predictions.jsonl"
    ...
    with jsonlines.open("standard_training.log", mode='a') as writer:
        params = vars(args)
