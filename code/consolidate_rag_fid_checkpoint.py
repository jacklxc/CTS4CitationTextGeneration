...
from transformers import AutoConfig, AutoTokenizer, RagConfig, BartForConditionalGeneration #, RagSequenceForGeneration, RagTokenForGeneration
...
def consolidate(
    ...
    generator_model = FiDBart.from_pretrained(generator_name_or_path)
    
    rag_model = model_class.from_pretrained_question_encoder_generator(
        question_encoder_name_or_path, generator_name_or_path, config=rag_config, generator_model=generator_model
    )
