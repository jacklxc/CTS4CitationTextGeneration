...
def get_sentence_bounds(paragraph, offset_mapping):
    bos_positions = []
    for token_i, (char_start, char_end) in enumerate(offset_mapping):
        if paragraph[char_start : char_end] == "[BOS]":
            bos_positions.append((token_i, char_start, char_end))
    sentence_bounds = []
    for i in range(len(bos_positions)-1):
        sentence_bounds.append({
            "char_start": bos_positions[i][1],
            "token_start": bos_positions[i][0],
            "char_end": bos_positions[i+1][1],
            "token_end": bos_positions[i+1][0],
        })
    sentence_bounds.append({
        "char_start": bos_positions[-1][1],
        "token_start": bos_positions[-1][0],
        "char_end": offset_mapping[-2][1],
        "token_end": len(offset_mapping)-1,
    })
    return sentence_bounds

def get_sent_span_mapping(span_sent_mapping, n_span):
    sent_span_mapping = [[] for i in range(n_span)]
    for sentence_i, sent in enumerate(span_sent_mapping):
        for span_i in sent:
            sent_span_mapping[span_i].append(sentence_i)
    return sent_span_mapping

def context_char_bound(i_span, sentence_bounds, span_sent_mapping, sent_span_mapping, context_window_size):
    sentence_indices = sent_span_mapping[i_span]
    context_sentence_start_idx = max([0, sentence_indices[0] - context_window_size])
    context_sentence_end_idx = min([len(span_sent_mapping)-1, sentence_indices[-1] + context_window_size])
    #print(i_span, context_sentence_start_idx, context_sentence_end_idx)
    context_char_start = sentence_bounds[context_sentence_start_idx]["char_start"]
    context_char_end = sentence_bounds[context_sentence_end_idx]["char_end"]
    return context_char_start, context_char_end
...
def truncate_string(text, tokenizer, max_length):
    token_ids = tokenizer(text, add_special_tokens = False)["input_ids"]
    if len(token_ids) > max_length:
        return tokenizer.decode(token_ids[:max_length])
    else:
        return text