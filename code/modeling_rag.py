...
class RagModel(RagPreTrainedModel):
    def __init__(
        ...
        **kwargs,
        ...
        if question_encoder is None:
            ...
            question_encoder = DPRQuestionEncoder(config = config.question_encoder)
    ...
    def forward(
        ...
        # (batch_size * n_doc, seq_len)
        #print("context_input_ids", context_input_ids.shape) 
        #print("context_attention_mask", context_attention_mask.shape)
        #print("encoder_outputs", encoder_outputs)
        #print("decoder_input_ids", decoder_input_ids.shape)
        #print("decoder_attention_mask",decoder_attention_mask)
        #assert()
        if type(self.generator) == FiDBart:
            gen_outputs = self.generator(
                input_ids=context_input_ids.view(doc_scores.shape[0], n_docs, -1, context_input_ids.shape[-1]),
                attention_mask=context_attention_mask.view(doc_scores.shape[0], n_docs, -1, context_attention_mask.shape[-1]),
            ...
            logits = gen_outputs.logits.repeat_interleave(n_docs, dim=0)
        else:
        ...
        # gen_outputs.logits: (batch_size * n_doc, seq_len)
...
class RagSequenceForGeneration(RagPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs  # needs kwargs for generation
        ...
        loss = None
        if labels is not None:
            #print(outputs.logits.shape) (batch_size*n_doc, n_vocab)
            #print(outputs.doc_scores.shape) (batch_size, n_doc)
            #print(decoder_input_ids.shape) (batch_size, target_len)
            #assert()
            loss = self.get_nll(
                outputs.logits,
                outputs.doc_scores,
        ...
        **model_kwargs
        ...
        for index in range(batch_size):
            ...
            output_sequences = self.generator.generate(
                generator_input_ids.unsqueeze(0) if type(self.generator) == FiDBart else generator_input_ids,
                **model_kwargs,
            ...
            if input_ids is not None:
                ...
                #print("individual_input_ids", individual_input_ids.shape)
                #print("individual_attention_mask", individual_attention_mask.shape)
                #print("individual_doc_scores", individual_doc_scores.shape)
                #print("output_sequences", output_sequences.shape)
    ...
    def get_nll(
        ...
        seq_logprobs = torch.nn.functional.log_softmax(seq_logits, dim=-1).view(
            doc_scores.shape[0], n_docs, -1, seq_logits.size(-1)
